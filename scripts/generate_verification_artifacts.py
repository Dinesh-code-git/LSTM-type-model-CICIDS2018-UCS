...
lr_manifest_path = ROOT / "artifacts/lr_set_a/lr_37fold_loeo_fold_manifest.csv"
...
features_path = ROOT / "artifacts/lr_set_a/set_a_features.json"
lr_per_fold_path = ROOT / "artifacts/lr_set_a/lr_detection_loeo_per_fold.csv"
...
# Exact fold comparison. The LR producer's compact manifest is not the frozen
# manifest: the corrected manifest contains benign controls and purge-adjusted
# train indices, while the LR manifest records episode-only test rows.
frozen_rows = []
...
coefficient_rows = []
for path in sorted((ROOT / "artifacts/lr").glob("lr_coefficient_audit_detection_fold*.csv")):
    fold_id = int(re.search(r"fold(\d+)", path.name).group(1))
    frame = pd.read_csv(path)
    feature_col = "feature" if "feature" in frame.columns else frame.columns[0]
    coef_col = "coefficient" if "coefficient" in frame.columns else frame.columns[1]
...
# Checkpoint status is separated from evaluation status because all four failed
# folds trained and wrote checkpoints before the empty-test prediction error.
pd.DataFrame(
    [
        {
            "fold_id": int(row["fold_id"]),
...
# Equivalence is intentionally false: LR did not use the frozen test populations.
...
write_json(OUT / "lr_run_provenance.json", {
    ...
    "command": "NOT RECORDED IN REPOSITORY EVIDENCE",
    "git_commit": "UNAVAILABLE: current Git history has no d7ca862 and producer files are untracked",
    ...
    "purge_implementation": "lstm.ucs.purge_and_embargo exists, but examples/run_lr_loeo.py does not call it",
    "generalized_purge_active_for_lr": False,
    "benign_sampling_rule": "none in LR producer; episode-only holdout",
    ...
    "raw_result_artifacts": [str(lr_per_fold_path.relative_to(ROOT)), str((ROOT / "artifacts/lr_set_a/lr_detection_loeo_predictions.csv").relative_to(ROOT))],
})
write_json(OUT / "lr_manifest_equivalence.json", {
    ...
    "lr_manifest_sha256": sha256(lr_manifest_path),
    "exact_train_and_test_population_match": bool(manifest_equivalence["exact_train_count_match"].all() and manifest_equivalence["exact_test_count_match"].all()),
...
write_json(OUT / "lr_runtime_configuration.json", {
    ...
    "purge_call_in_producer": False,
    "manifest_load_call_in_producer": False,
})
write_json(OUT / "lr_verification_status.json", {
    "lr_status": "INVALID",
    "reasons": [
        "LR producer does not load the final frozen corrected manifest",
        "LR producer does not invoke generalized 35-minute purge/embargo",
        "LR fold 0 has 10 test rows while frozen corrected fold 0 has 20 rows with benign controls",
    ],
...
print(json.dumps({
    ...
    "lstm_completed_training_folds": int(completion["training_completed"].sum()),
